        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {packages.map((pkg, index) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              className={`relative p-8 rounded-[2rem] transition-all ${pkg.highlighted
                ? "bg-slate-900 text-white shadow-2xl scale-105 z-10"
                : "bg-white text-slate-900 border border-slate-200"
                }`}
            >
              ...
              <div className="flex flex-col mb-8">
                ...
                <span className={`text-xs mt-1 font-medium ${pkg.highlighted ? "text-slate-400" : "text-slate-500"}`}>
                  Range: {pkg.range}
                </span>
              </div>
              <ul className="space-y-4 mb-10">
                {pkg.features.map((feature, i) => (
                  <li key={i} className="flex items-center text-sm">
                    <svg className={`w-5 h-5 mr-3 flex-shrink-0 ${pkg.highlighted ? "text-indigo-400" : "text-indigo-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" />
                    </svg>
                    <span className={pkg.highlighted ? "text-slate-300" : "text-slate-600"}>{feature}</span>
                  </li>
              ...
              <button className={`w-full py-4 rounded-xl font-bold text-sm transition-all ${pkg.highlighted
                ? "bg-indigo-600 hover:bg-indigo-500 text-white"
                : "bg-slate-100 hover:bg-slate-200 text-slate-900"
                }`}>
                Start Project
